# 추가
# activity-outbox-connector를 추가합니다.
...
# market-data-outbox-connector 를 추가합니다.
curl -i -X POST -H "Content-Type: application/json" --data '{
    "name": "market-data-outbox-connector",
    "config": {
        "connector.class": "io.debezium.connector.postgresql.PostgresConnector",
        "database.hostname": "postgres",
        "database.port": "5432",
        "database.user": "postgres",
        "database.password": "postgres",
        "database.dbname": "market_data",
        "database.server.name": "market_data",
        "topic.prefix": "market_data",
        "table.include.list": "public.outbox",
        "slot.name": "debezium_market_data_slot",
        "plugin.name": "pgoutput",
        "tombstones.on.delete": "false",
        "transforms": "outbox",
        "transforms.outbox.type": "io.debezium.transforms.outbox.EventRouter",
